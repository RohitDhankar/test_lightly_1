...
import numpy as np
import lightly
from lightly.models.modules.heads import SimSiamPredictionHead ,SimSiamProjectionHead #lightly/models/modules/heads.py
...
#TODO -- SimSiamProjectionHead == See Layers Structure in oWN LOGS FIle -- 
# (projection_head): SimSiamProjectionHead( (layers): Sequential(
...
path_to_data = './datasets/input_dir/imgs/Faces_easy/'
path_to_data_test = './datasets/input_dir/imgs/Faces_easy/test_data/'

# Setup data augmentations and loaders
# ------------------------------------
# Since we're working on satellite images, it makes sense to use horizontal and
# vertical flips as well as random rotation transformations.  # We apply weak color # jitter to ---learn an invariance---- of the model with respect to slight changes in
...
print("----fname--dataset_train_simsiam---",fname) #
...
caltech_train_batch_size = 6 ## 52 TIFF FIles here -- kaggle_road_data/tiff/test
dataloader_train_simsiam = torch.utils.data.DataLoader(dataset_train_simsiam,
    batch_size=caltech_train_batch_size,
    shuffle=True,
...
dataset_test = lightly.data.LightlyDataset(input_dir=path_to_data_test,transform=test_transforms)

# create a dataloader for embedding
caltech_test_batch_size = 6 
dataloader_test = torch.utils.data.DataLoader(
    dataset_test,
    batch_size=caltech_test_batch_size,
    shuffle=False,
    drop_last=False,
    num_workers=num_workers
)
...
class SimSiam(nn.Module):
    """ # TODO -- https://github.com/lightly-ai/lightly/blob/master/lightly/models/simsiam.py
        SimSiamProjectionHead == See Layers Structure in oWN LOGS FIle -- 
        (projection_head): SimSiamProjectionHead( (layers): Sequential(
    """
    def __init__(
    ...
    def forward(self, x):
        ...
        print("----Type(p--from--prediction_head-",type(p_prediction_head))
        # stop gradient
...
for e in range(epochs):
    print("----type(dataloader_train_simsiam--CALTECH FACES--",type(dataloader_train_simsiam))
    ...
    print(f'[Epoch__CALTECH FACES__ {e:3d}] '
        f'Loss__CALTECH FACES__ = {avg_loss:.2f} | '
        f'Collapse Level__CALTECH FACES__: {collapse_level:.2f} / 1.00')
...
with torch.no_grad():
    for i, (x, _, fnames) in enumerate(dataloader_test):
        print("--filenmes---for--Embeddings__CALTECH FACES__",fnames)
        # move the images to the gpu
        x = x.to(device)
        # embed the images with the pre-trained backbone
        y = model.backbone(x).flatten(start_dim=1)
        print("---type-embeddings--y--__CALTECH FACES-",type(y))
...
#print("---type-embeddings--_CALTECH FACES___>>",type(embeddings)) ## LIST 
print("---LIST---embeddings--_CALTECH FACES___>>",embeddings) ## LIST 
...
print("---type-embeddings-_CALTECH FACES__-3--->>",type(embeddings))
...
from sklearn import random_projection ## https://scikit-learn.org/stable/modules/random_projection.html

# Transform embeddings using UMAP and rescale them to fit in the  [0, 1] square.

# for scatter plot we transform EMBEDDINGS (NOT-images) to a two-dimensional vector space using a 
...
def get_scatter_plot_with_thumbnails():
    """Creates a scatter plot with image overlays__CALTECH FACES___.
    """
    # initialize empty figure and add subplot
    fig = plt.figure()
    fig.suptitle('Scatter Plot__CALTECH FACES_')
    ...
    for idx in shown_images_idx:
        ...
        print("-_CALTECH FACES___>>_Images to Plot Local Path---",path)
        img = Image.open(path)
    ...
    plot_name = "CALTECH_FACES__"
    plt.savefig('./datasets/output_dir/'+str(plot_name)+"_.pdf", bbox_inches='tight') # 


# get a scatter plot with thumbnail overlays
get_scatter_plot_with_thumbnails()
print("--ScatterPlot--Saved--CALTECH_FACES__>>")
...
def plot_nearest_neighbors_3x3(example_image: str, i: int):
    ...
    print("--example_idx----",example_idx) ## INDEX of Images in own defined List above -- example_images
    # get distances to the cluster center
    print("---embeddings[example_idx]----",embeddings[example_idx])
    ...
    for plot_offset, plot_idx in enumerate(nearest_neighbors):
        ...
        file_name = fname.rsplit("/",1)[1] # upto 
        print("--plot_nn--Example Image--filenames---caltech_img_names-",file_name)
        example_image_name = file_name.rsplit("_",1)[0] # 
        print("--plot_nn--Example Image--filenames---caltech_img_names-",example_image_name)

        # plot_name = "nn_caltech_imgs"
        ...
        #     file_name = fname.rsplit("/",1)[1] # upto 
        #     print("--plot_nn--Example Image--filenames---caltech_img_names-",file_name)
        #     example_image_name = file_name.rsplit("_",1)[0] # caltech_img_names
        #     print("--plot_nn--Example Image--filenames---caltech_img_names-",example_image_name)
        # else:
        #     pass
        ...
        if plot_offset == 0:
            ...
            plt.axis("off")
            #plt.savefig('./datasets/output_dir/_example_'+str(example_image_name)+"_.pdf", bbox_inches='tight')
        else:
            ...
            plt.savefig('./datasets/output_dir/_nn_2_'+str(str_name)+"_.pdf", bbox_inches='tight')
